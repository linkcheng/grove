...
- 功能分支开发期间，可按独立功能点创建多个清晰的本地 commit，并可 push 功能分支。
- 合并到 `main` 前，必须先将该功能分支整理/squash 为单一 commit，再合并到 `main`。
- 后续新功能/工作包默认使用独立 git worktree 开发；当前进行中的 WS-5 作为一次性例外，延续现有工作区和现有功能分支，不中途迁移 worktree。
- “可 push”不等同于自动获得 PR/merge 授权；PR/merge 仍需用户明确要求。
