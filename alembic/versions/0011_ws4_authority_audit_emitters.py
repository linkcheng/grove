"""WS-4 authority audit emitters with closed role/source/schema binding."""
...
revision: str = "ws4_authority_audit_emitters"
down_revision: str | None = "ws4_recon_helpers"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

_SIGNATURE = "TEXT, UUID, UUID, TEXT, UUID, TEXT, JSONB"


def _emit_sql(*, ws4_roles: bool) -> str:
    role_guard = (
        "session_user NOT IN ('grove_runtime', 'grove_api', 'grove_projection')"
        if ws4_roles
        else "session_user <> 'grove_runtime'"
    )
    source_binding = (
        """
        ...
        """
        if ws4_roles
        else "expected_source := 'grove.runtime_worker';"
    )
    return f"""
...
def upgrade() -> None:
    op.execute(_emit_sql(ws4_roles=True))
    op.execute(f"REVOKE ALL ON FUNCTION grove_emit_runtime_events({_SIGNATURE}) FROM PUBLIC")
    op.execute(
        f"GRANT EXECUTE ON FUNCTION grove_emit_runtime_events({_SIGNATURE}) "
        "TO grove_runtime, grove_api, grove_projection"
    )


def downgrade() -> None:
    op.execute(f"REVOKE ALL ON FUNCTION grove_emit_runtime_events({_SIGNATURE}) FROM grove_api, grove_projection")
    op.execute(_emit_sql(ws4_roles=False))
    op.execute(f"REVOKE ALL ON FUNCTION grove_emit_runtime_events({_SIGNATURE}) FROM PUBLIC")
    op.execute(f"GRANT EXECUTE ON FUNCTION grove_emit_runtime_events({_SIGNATURE}) TO grove_runtime")
